...
function Board() {
    ...
    function handleSquareInput(letter, row, col, inputLocation) {
        ...
        if (inputLocation.current[row * DIMENSIONS + col].value == "") {
            ...
            if (
                ...
                inputLocation.current[
                    row * DIMENSIONS + col + DIMENSIONS * 2
                ].focus();
            } else {
                inputLocation.current[
                    row * DIMENSIONS + col + DIMENSIONS
    ...
    function checkForCollisions(wordToAdd, currentBoard) {
        ...
        for (let i = 0; i < DIMENSIONS; i++) {
            for (let j = 0; j < DIMENSIONS; j++) {
                if (
                    wordToAdd.includes(
                        currentBoard[i].CURRENT_ROW[j].KEY_CHARACTER
                    )
                ) {
                    collisions.push({
                        row: i,
        ...
        if (
            ...
            for (let i = 1; i <= firstPart.length; i++) {
                if (
                    ...
                    currentBoard[collision.row].CURRENT_ROW[collision.col - i]
                        .AVAILABLE == true &&
                    currentBoard[collision.row + 1].CURRENT_ROW[
                        collision.col - i
                    ].KEY_CHARACTER == "*" &&
                    currentBoard[collision.row - 1].CURRENT_ROW[
                        collision.col - i
        ...
        if (
            ...
            for (let i = 1; i <= firstPart.length; i++) {
                if (
                    ...
                    currentBoard[collision.row - i - 1].CURRENT_ROW[
                        collision.col
                    ].AVAILABLE == true
                ) {
                    canInsert = true;
    ...
    function setUpBoard(
        ...
        const randomAncestor =
            SORTED_CLUE_LIST[
                Math.floor(Math.random() * SORTED_CLUE_LIST.length)
            ];
        ...
        if (canInsertWord) {
            ...
            for (let i = 0; i < splitWord.length; i++) {
                currentBoard[rowIndex].CURRENT_ROW[colIndex + i].KEY_CHARACTER =
                    splitWord[i];
                currentBoard[rowIndex].CURRENT_ROW[
                    colIndex + i
                ].AVAILABLE = false;
            }
    ...
    return !loading ? (
        <>
            ...
            <Modal
                ...
                onClose={() => setPuzzleIsCorrect(false)}
            >
